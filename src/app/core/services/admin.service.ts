...
export class AdminService {
  ...
  /**
   * Get manager/officer by ID.
   * Backend: GET /api/admin/users/:id
   */
  getAdminUserById(userId: string): Observable<AdminUser> {
    return this.http.get<AdminUser>(`${ADMIN_API}/${userId}`, { withCredentials: true });
  }

  /**
   * Change status (activate, block, unblock) for a manager/officer.
   * Backend: PUT /api/admin/users/:id/status
   * @param userId - The user ID
   * @param status - 'active' or 'blocked' (cannot revert to 'pending')
   */
  updateUserStatus(userId: string, status: 'active' | 'blocked' | 'pending'): Observable<any> {
    return this.http.put(`${ADMIN_API}/${userId}/status`, { status }, { withCredentials: true });
  }
